...
begin
  ...
  try
    ...
    FPrefixID := nIni.ReadString(Name, 'IDPrefix', 'GYS');
...
//Desc: 随机编号
procedure TfFormProvider.EditIDPropertiesButtonClick(Sender: TObject;
  AButtonIndex: Integer);
begin
...
begin
  ...
  begin
    ...
    nStr := Format(nStr, [sTable_Provider, EditID.Text]);
    //查询编号是否存在
    ...
    with FDM.QueryTemp(nStr) do
     ...
     begin
       EditID.SetFocus;
       ShowMsg('该编号的供应商已经存在', sHint); Exit;
     end;

    nSQL := MakeSQLByForm(Self, sTable_Provider, '', True, GetData);
  end else
  begin
    EditID.Text := FRecordID;
    nStr := 'P_ID=''' + FRecordID + '''';
    nSQL := MakeSQLByForm(Self, sTable_Provider, nStr, False, GetData);
  end;
  ...
  try
    ...
    begin
      ...
      begin
        ...
        nSQL := Format(nSQL, [sTable_ExtInfo, sFlag_ProviderItem, FRecordID]);
        FDM.ExecuteSQL(nSQL);
      end;
      ...
      begin
        ...
        nStr := Format(nSQL, [sTable_ExtInfo, sFlag_ProviderItem,
